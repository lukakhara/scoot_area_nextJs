...
import { SiFacebook as Facebook, SiInstagram as Instagram, SiLinkerd as Linkedin, SiYoutube as Youtube  } from "@icons-pack/react-simple-icons";
      ...
      <main>
        ...
        <section className="mx-auto max-w-[1400px] px-5 pt-8 pb-12 lg:pt-12">
          ...
          <div className="mt-5 space-y-4 text-sm leading-relaxed text-muted-foreground sm:text-[15px]">
            ...
            <p>
              <strong className="text-foreground">ScootArea</strong> — ეს არის გზა, რომელიც შენით
              იწყება, მაგრამ მარტო არასდროს გატარებ.
            </p>
          </div>
          ...
          <div className="mt-10 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 sm:flex sm:justify-between">
            <div className="flex min-w-0 flex-wrap items-center gap-4">
              {[Facebook, Youtube, Instagram, Linkedin, Send].map((Icon, i) => (
                <a key={i} href="#" aria-label="social" className="hover:text-primary">
        ...
        <section className="mx-auto max-w-[1400px] px-5 pb-14">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4">
            ...
            <div className="flex shrink-0 items-center gap-2">
              <span className="hidden text-xs text-muted-foreground sm:inline">ყველას ნახვა</span>
              <button
                aria-label="უკან"
          ...
          <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i}>
                <Placeholder className="aspect-square w-full rounded-lg" label="logo" />
        ...
        <section className="mx-auto max-w-[1400px] px-5 pb-16">
          <h2 className="text-xl font-extrabold tracking-tight uppercase sm:text-2xl">გალერეა</h2>
          <div className="mt-5 grid grid-cols-2 gap-4 lg:grid-cols-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <Placeholder key={i} className="aspect-[4/3] w-full rounded-xl" label="gallery" />
            ))}
          </div>
        </section>
      </main>
